  ...
  return [
    {
      ...
      label: "Muse Setup",
      description:
        "Install and configure Muse. Run this first — it installs the CLI, " +
      ...
      async execute() {
        ...
        if (authStatus === "authenticated") {
          ...
          if (status.stdout.includes("Active")) {
            ...
            lines.push("Telemetry: Not active. Run the muse_enable tool to start collecting.");
          }

          lines.push("");
          lines.push("Setup complete. Muse is fully operational.");
          lines.push("");
          ...
          lines.push("");
          lines.push("Without login, Muse still works in local-only mode (basic session data).");
        }
  ...
  return [
    {
      name: "muse_enable",
      label: "Muse Enable",
      description: "Enable Muse telemetry collection for Claude, Cursor, and Codex.",
      parameters: Type.Object({}),
      async execute() {
    ...
    },
    {
      name: "muse_disable",
      label: "Muse Disable",
      description: "Disable Muse telemetry collection.",
      parameters: Type.Object({}),
      async execute() {
    ...
    },
    {
      ...
      label: "Muse Status",
      description:
        "Show Muse telemetry collection status including sync state and connected tools.",
      parameters: Type.Object({}),
      async execute() {
    ...
    },
    {
      name: "muse_version",
      label: "Muse Version",
      description: "Show Muse CLI version and build configuration.",
      parameters: Type.Object({}),
      async execute() {
...
function authTools(): ToolDef[] {
  return [
    {
      name: "muse_auth_status",
      label: "Muse Auth Status",
      description:
        "Check if the user is authenticated with Muse. Returns auth state without triggering login.",
      parameters: Type.Object({}),
    ...
    },
    {
      name: "muse_logout",
      label: "Muse Logout",
      description: "Remove Muse authentication credentials.",
      parameters: Type.Object({}),
      async execute() {
...
function searchTools(): ToolDef[] {
  return [
    ...
    {
      name: "muse_query_sessions",
      label: "Muse Query Sessions",
      description:
    ...
    {
      name: "muse_query_insights",
      label: "Muse Query Insights",
    ...
    {
      name: "muse_query_events",
      label: "Muse Query Events",
...
function sessionTools(): ToolDef[] {
  return [
    {
      name: "muse_sessions_list",
      label: "Muse Sessions List",
    ...
    {
      name: "muse_sessions_read",
      label: "Muse Sessions Read",
    ...
    {
      name: "muse_sessions_search",
      label: "Muse Sessions Search",
...
function kbTools(): ToolDef[] {
  return [
    {
      name: "muse_kb_search",
      label: "Muse KB Search",
      description: "Search the Muse knowledge base for relevant documents.",
      parameters: Type.Object({
        query: Type.String({ description: "Search query" }),
      }),
      async execute(_id, params) {
    ...
    },
    {
      name: "muse_kb_list",
      label: "Muse KB List",
      description: "List all documents in the Muse knowledge base.",
      parameters: Type.Object({}),
      async execute() {
    ...
    },
    {
      name: "muse_kb_save",
      label: "Muse KB Save",
      description: "Save content to the Muse knowledge base.",
      parameters: Type.Object({
...
function orchestrationTools(): ToolDef[] {
  return [
    {
      name: "muse_leader_start",
      label: "Muse Leader Start",
    ...
    {
      name: "muse_orchestration_status",
      label: "Muse Orchestration Status",
...
function circuitTools(): ToolDef[] {
  return [
    {
      name: "muse_circuit_list",
      label: "Muse CIRCUIT List",
    ...
    {
      name: "muse_circuit_spawn",
      label: "Muse CIRCUIT Spawn",
    ...
    {
      name: "muse_circuit_status",
      label: "Muse CIRCUIT Status",
    ...
    {
      name: "muse_circuit_metrics",
      label: "Muse CIRCUIT Metrics",
    ...
    {
      name: "muse_circuit_auto",
      label: "Muse CIRCUIT Auto",
      description:
  ...
  configSchema: {
    ...
    properties: {
      ...
      autoCapture: {
        type: "boolean" as const,
        default: true,
        description: "Automatically capture Moltbot conversations to TRIBE KB",
      },
      autoSync: {
        type: "boolean" as const,
        default: false,
  ...
  register(api: ClawdbotPluginApi) {
    ...
    checkAuthStatus().then((status) => {
      if (status === "not-installed") {
        api.logger.warn(
          "muse: TRIBE CLI not found. Use the muse_setup tool to install.",
        );
      } else if (status === "not-authenticated") {
        api.logger.warn(
          "muse: Not authenticated. Run 'tribe login' in your terminal, or use muse_setup.",
        );
        api.logger.info(
          "muse: Active in local-only mode (limited session data).",
        );
      } else {
        api.logger.info("muse: TRIBE connected and authenticated.");
        api.logger.info(
          `muse: autoContext=${pluginCfg?.autoContext ?? true}, ` +
      ...
      api.registerTool(
        {
          ...
          async execute(toolCallId: string, params: Record<string, unknown>) {
            // muse_setup handles its own install/auth logic
            if (tool.name === "muse_setup") {
              return tool.execute(toolCallId, params);
            }
            const status = await checkAuthStatus();
            if (status === "not-installed") {
              return textResult(
                "TRIBE CLI is not installed. Use the muse_setup tool to install it automatically.",
              );
            }
            // Let tools that work without auth proceed
            const noAuthRequired = [
              "muse_status", "muse_version", "muse_enable", "muse_disable", "muse_auth_status",
            ];
            if (status === "not-authenticated" && !noAuthRequired.includes(tool.name)) {
              return textResult(
                ...
                "Or use the muse_setup tool for guided setup.\n\n" +
                "Without login, Muse works in local-only mode (basic session data).",
              );
            }
            return tool.execute(toolCallId, params);
          },
    ...
    api.on("before_agent_start", async (event) => {
      ...
      try {
        ...
        if (!context) {
          api.logger.debug("muse: no relevant context found for this prompt.");
          return;
        }
        ...
        api.logger.info(
          `muse: injecting context (${parts.join(", ")}) — ${context.length} chars`,
        );
        return { prependContext: context };
      } catch (err) {
        api.logger.warn(`muse: context injection failed: ${String(err)}`);
      }
    });
    ...
    api.registerService({
      ...
      start() {
        ...
        api.logger.info("muse: background sync enabled (every 5 minutes).");
        syncInterval = setInterval(
          async () => {
            try {
              await run(["-force"], { timeout: "slow" });
              invalidateCache();
              api.logger.debug("muse: background sync completed.");
            } catch (err) {
              api.logger.warn(`muse: background sync failed: ${String(err)}`);
            }
          },
          5 * 60 * 1000,
        );
